import { toLineSegments } from './3.mjs';
import { toCornerCoords } from './3.mjs';
import { toVectorMoves } from './3.mjs';
import { doLineSegmentsIntersect } from './3.mjs';
import { probe } from './probe.mjs';
import { getIntersecionPoint } from './3.mjs';
import { vectorAdd } from './3.mjs';

const stepsOnPathToPoint = ([x, y], pathMoves) => {
  let point = [0, 0];
  let steps = 0;
  while (!(point[0] == x && point[1] == y) && pathMoves.length) {
    // intersects vertically in this move
    if (point[0] === x && Math.sign(y-point[1]) != Math.sign(y - point[1]-pathMoves[0][1])) {
      return steps + Math.abs(y - point[1]);
    } else if (point[1] === y && Math.sign(x - point[0]) != Math.sign(x - point[0]-pathMoves[0][0])) {
      return steps + Math.abs(x - point[0]);
    } else {
      point = vectorAdd(point, pathMoves[0]);
      steps += Math.abs(pathMoves[0][0]) + Math.abs(pathMoves[0][1]);
      pathMoves = pathMoves.slice(1);
    }
  }
  if (!pathMoves.length) console.log('fucked');
  return steps;
};

const summedStepsFromOrigin = ([x, y], [moves1, moves2]) => {
  return stepsOnPathToPoint([x, y], moves1) + stepsOnPathToPoint([x, y], moves2);
};

const intersectionSteps = input => {
  const vectorMoves = toVectorMoves(input);
  const [path1, path2] = toLineSegments(toCornerCoords(vectorMoves));
  const intersections = [];
  ...
  return Math.min(
    ...intersections
      .filter(([x, y]) => !(x === 0 && y === 0))
      .map(i => summedStepsFromOrigin(i, vectorMoves))
  );
};
...
probe(
  ...
  testCases.map(
    ({ input, output }) =>
      probe('actual distance', intersectionSteps(input)) === probe('expected distance', output)
  )
);
