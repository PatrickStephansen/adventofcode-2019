...
export const vectorAdd = (...vectors) =>
  vectors[0].map((_, index) => vectors.reduce((sum, v) => sum + v[index], 0));
export const toVectorMoves = s =>
  s
    .split('\n')
    .map(pathString =>
      pathString
        .split(',')
        .map(moveString =>
          (([direction, ...numbers] = moveString, distance = +numbers.join``) =>
            direction === 'R'
              ? [distance, 0]
              : direction === 'L'
              ? [-distance, 0]
              : direction === 'U'
              ? [0, distance]
              : [0, -distance])(moveString)
        )
    );
export const toCornerCoords = vectorMoves =>
  vectorMoves.map(path =>
    path.reduce(
      (traversal, nextMove) => [...traversal, vectorAdd(traversal[traversal.length - 1], nextMove)],
      [[0, 0]]
    )
  );
export const toLineSegments = coordsLists =>
  coordsLists.map(path => path.map((point, index, p) => [p[index - 1], point]).filter(x => x[0]));
const distanceFromOrigin = v => v.reduce((a, c) => Math.abs(a) + Math.abs(c));
const isVertical = ([startPoint, endPoint]) => startPoint[0] == endPoint[0];
export const doLineSegmentsIntersect = (l1, l2) =>
  isVertical(l1) === isVertical(l2)
    ? false
    : isVertical(l1) && !isVertical(l2)
    ? l1[0][0] >= Math.min(l2[0][0], l2[1][0]) &&
      l1[0][0] <= Math.max(l2[0][0], l2[1][0]) &&
      l2[0][1] <= Math.max(l1[0][1], l1[1][1]) &&
      l2[0][1] >= Math.min(l1[0][1], l1[1][1])
    : doLineSegmentsIntersect(l2, l1);
export const getIntersecionPoint = (l1, l2) =>
  isVertical(l1) ? [l1[0][0], l2[0][1]] : getIntersecionPoint(l2, l1);

const intersectionDistance = input => {
  const [path1, path2] = toLineSegments(toCornerCoords(toVectorMoves(input)));
  const intersections = [];
  for (const l1 of path1) {
    for (const l2 of path2) {
      if (doLineSegmentsIntersect(l1, l2)) {
        intersections.push(probe('intersection:', getIntersecionPoint(l1, l2)));
      }
    }
  }
  return Math.min(
    ...intersections.filter(([x, y]) => !(x === 0 && y === 0)).map(distanceFromOrigin)
  );
};
